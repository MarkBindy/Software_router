...
```
# 磁盘扩容
确认是 SquashFS 固件，只要分区表在底层被拉满，我们根本不需要格式化，也不需要创建 sda3 ，直接利用系统自带的 loop0 回环设备就能实现无损在线扩容！
手动强制它检测并修复 GPT 尾部指针，然后再执行扩容。请严格输入以下指令：
第一步：登入后台终端
```
...
print
```
第五步：💡关键点：输入 print 后，parted 会强制扫描整块磁盘，这时它绝对会弹出提示：Warning: Not all of the space available to /dev/sda appears to be used... Fix/Ignore? 看到这个提示后，请立刻输入下方命令并回车
```
Fix
```
第六步：第二分区在底层完美扩容至100%
